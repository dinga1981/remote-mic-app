...
Typeless 等点按 Fn 开始、再次点按结束的语音工具，与小米蓝牙遥控器 2 Pro 默认的 Fn 长按行为不兼容。在“连接与语音”中开启“语音键使用点按式触发”后，无线麦会在语音流开始和排空结束时各发送一次 Fn 点按。Typeless 和无线麦仍需选择同一个回环设备，并需授予无线麦“辅助功能”权限。
...
首次设置时选择“macOS 系统听写”，无线麦会在语音流开始时发送系统 `Fn-D` 听写快捷键，缓存并补回快捷键生效前的开头音频；松开语音键并排空音频后，再发送一次 `Fn-D` 停止听写。该模式不依赖“连续按两次 Fn”的键盘设置，但仍需要辅助功能权限，并要求 macOS 听写与无线麦选择同一个回环音频设备。
